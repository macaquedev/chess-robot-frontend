...
class RandomBoard:
    def __init__(self, num_white_pawns=8, num_white_knights=2, num_white_bishops=2,
                 num_white_rooks=2, num_white_queens=1, num_white_kings=1,
                 num_black_pawns=8, num_black_knights=2, num_black_bishops=2,
                 num_black_rooks=2, num_black_queens=1, num_black_kings=1, seed=1):
        self.rotation = 0
        self.seed = seed
        random.seed(seed)
        self.pieces = []
...
def _capture_chessboard(base_path, random_board, frame):
    ...
    if not cv2.imwrite(path, frame):
        raise Exception(f"Imwrite failed at {path}")
...
def main():
    ...
    cam = ChessCamera(config.CAMERA_INDEX)
    ...
    while not end:
        ...
        for i in range(1, 8):
            ...
            cv2.line(img, (0, mx_pt), (BOARD_SIZE, mx_pt), (0, 0, 255))
            cv2.line(img, (mx_pt, 0), (mx_pt, BOARD_SIZE), (0, 0, 255))
        ...
        for i in range(64):
            ...
            if piece is not None:
                text = str(piece)
                cv2.putText(img, text, (x + 10, y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
        ...
        if key == ord('r'):
            ...
            _capture_chessboard(args.base_path, rb, saved_img)
        elif key == ord('q'):
            end = True
